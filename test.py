...
for xd in range(10000):
	...
	a='Mozilla/5.0 (Linux; Android 4.1.2;'	b=random.randrange(1, 9)
...
def banner():
	...
	vprint(panel(f'''      ''',style=f'{warna_warni_rich_cerah}',width=50)) 
...
def bagi():
	...
	inf.append(Panel(f'{P2}AUTHOR : Arafat Bin Mahin', style=f'{warna_warni_rich_cerah}'))
	...
	inf.append(Panel(f"{P2} AUTHOR: MAHIN-xyz", style=f"{warna_warni_rich_cerah}"))
...
def login():
	...
	if cukuf in ["help","Help","HELP"]:
		...
		x=f"{P2}whatsapp admin *--> {H2}01925798757 {P2}harap chat klo memang ada yang error\nini klo gak bisa diarahin ke whastapp admin yakk"
